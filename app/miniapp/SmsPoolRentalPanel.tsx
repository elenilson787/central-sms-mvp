...
export default function SmsPoolRentalPanel() {
  ...
  useEffect(() => {
    if (!rentalId) return;
    void (async () => {
      setDetailsLoading(true); setError(null); setDetails(null); setSelectedService(null); setQuote(null);
      try {
  ...
  return <section className={styles.catalogSection}>
    ...
    {!loading && rentals.length > 0 && <div className={styles.filters}>
      ...
      {details && <>
        ...
        <select id="rental-days" className={styles.select} value={days ?? ""} onChange={(event) => { setDays(Number(event.target.value)); setQuote(null); }}>
        ...
        <div className={styles.quickSearches} aria-label="Buscas rápidas de aluguel">
          <span>Exemplos:</span>
          {QUICK_SEARCHES.map((item) => <button key={item} className={styles.quickSearchButton} type="button" onClick={() => { setSearch(item); setSelectedService(null); setQuote(null); }}>{item}</button>)}
        </div>
        ...
        {query.length >= 2 && <div className={styles.offerList}>
          ...
          {filteredServices.map((service) => <button
            ...
            onClick={() => { setSelectedService(service); setSearch(service.name); setQuote(null); }}
          >
            {selectedService?.id === service.id ? "✓ " : ""}{service.name}
          </button>)}
        </div>}

        {selectedService && days && <button className={styles.primaryButton} type="button" disabled={quoteLoading} onClick={() => void checkAvailability()}>
          {quoteLoading ? "Consultando…" : `Consultar disponibilidade para ${selectedService.name}`}
        </button>}
      </>}
    </div>}

    {quote && <article className={styles.offerCard}>
