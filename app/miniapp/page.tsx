...
export default function TelegramMiniAppPage() {
  ...
  const countries = useMemo(() => {
    ...
    catalog.forEach((offer) => values.set(offer.country, offer.countryName));
  ...
  const filteredOffers = useMemo(() => {
    ...
    return catalog.filter((offer) => {
      ...
      if (query && !`${offer.countryName} ${offer.label} ${offer.product}`.toLowerCase().includes(query)) return false;
      return true;
    });
  ...
  async function openQuote(offer: CatalogOffer) {
    ...
    try {
      ...
      popup("Não foi possível calcular", cause instanceof Error ? cause.message : "Erro ao gerar orçamento.");
    } finally {
  ...
  function simulateConfirmation() {
    ...
    popup(
      "Simulação concluída",
      `Fluxo validado para ${quote.offer.label} em ${quote.offer.countryName}, total ${formatMoney(quote.quote.salePriceCents)}. Nenhuma compra, reserva ou débito foi executado.`,
    );
    setSelectedOffer(null);
    setQuote(null);
  }
      ...
      <div className={styles.container}>
        <header className={styles.header}>
          <div className={styles.brand}>
            {view !== "home" && (
              <button className={styles.back} type="button" onClick={() => setView("home")} aria-label="Voltar">←</button>
            )}
            <div className={styles.logo}>📲</div>
            <div>
              <h1 className={styles.title}>{view === "home" ? "Central SMS" : view === "catalog" ? "Catálogo" : "Ativações"}</h1>
          ...
          <>
            ...
            <section className={styles.grid} aria-label="Ações principais">
              <button className={styles.action} type="button" onClick={goCatalog}>
                <div className={styles.actionIcon}>📱</div><span className={styles.actionTitle}>Comprar número</span><span className={styles.actionText}>Escolher país, serviço e tipo de ativação</span>
              </button>
              <button className={styles.action} type="button" onClick={goCatalog}>
                <div className={styles.actionIcon}>🌎</div><span className={styles.actionTitle}>Países e serviços</span><span className={styles.actionText}>Catálogo, estoque e preço final</span>
              </button>
              <button className={styles.action} type="button" onClick={() => popup("Recarregar", "O fluxo PIX será conectado na próxima etapa. Nenhuma cobrança foi criada.")}>
                <div className={styles.actionIcon}>💳</div><span className={styles.actionTitle}>Recarregar</span><span className={styles.actionText}>Adicionar saldo via PIX</span>
              </button>
              <button className={styles.action} type="button" onClick={() => setView("activations")}>
                <div className={styles.actionIcon}>📋</div><span className={styles.actionTitle}>Ativações</span><span className={styles.actionText}>Status, números e histórico recente</span>
              </button>
            </section>

            <section className={styles.section}>
              <div className={styles.sectionHeader}><h2 className={styles.sectionTitle}>Ativações recentes</h2><span className={styles.badge}>{recentList.length}/5</span></div>
        ...
        {session && view === "catalog" && (
          <section className={styles.catalogSection}>
            <div className={styles.previewNotice}>
              ...
              <span>Estoque e preços abaixo servem para validar a experiência. Nenhuma oferta reserva número em provider externo.</span>
            </div>
            ...
            <div className={styles.filters}>
              ...
              <div className={styles.filterRow}>
                <select className={styles.select} value={country} onChange={(event) => setCountry(event.target.value)}>
                  <option value="all">Todos os países</option>
                  {countries.map(([id, name]) => <option key={id} value={id}>{name}</option>)}
                </select>
                <select className={styles.select} value={kind} onChange={(event) => setKind(event.target.value as "all" | NumberKind)}>
                  <option value="all">Todos os tipos</option>
                  <option value="ONE_TIME_SMS">Ativação curta</option>
                  <option value="TEMPORARY_HOSTING">Número temporário</option>
                </select>
              </div>
            </div>
            ...
            {!catalogLoading && !catalogError && (
              <div className={styles.offerList}>
                {filteredOffers.length === 0 && <div className={styles.empty}>Nenhuma oferta para esses filtros.</div>}
                {filteredOffers.map((offer) => (
                  <article className={styles.offerCard} key={offer.id}>
                    <div className={styles.offerTop}>
                      <div><span className={styles.demoBadge}>DEMO</span><h3>{offer.label}</h3><p>{offer.countryName} · {kindLabel(offer.kind)}</p></div>
                      <div className={styles.offerPrice}>{formatMoney(offer.quote.salePriceCents)}</div>
                    </div>
                    <p className={styles.offerDescription}>{offer.description}</p>
                    <div className={styles.offerMeta}>
                      <span>Estoque: {offer.stock}</span>
                      {offer.successRate !== undefined && <span>Taxa: {offer.successRate}%</span>}
                      <span>Markup: {offer.quote.markupPercent}% + {formatMoney(offer.quote.markupFixedBrlCents)}</span>
                    </div>
        ...
        {session && view === "activations" && (
          <section className={styles.section}>
            <div className={styles.sectionHeader}><h2 className={styles.sectionTitle}>Suas ativações</h2><span className={styles.badge}>{recentList.length} recentes</span></div>
            <ActivationList activations={recentList} currency={session.wallet.currency} />
            <p className={styles.helper}>A sessão atual carrega as 5 ativações mais recentes. Paginação e SMS detalhado entram quando o provider real for conectado.</p>
          </section>
        )}

        <footer className={styles.footer}>
          Identidade validada no servidor pelo initData assinado do Telegram.<br />
          Compras reais permanecem desativadas até validação comercial dos providers.
        </footer>
      </div>

      {selectedOffer && (
        <div className={styles.modalBackdrop} role="presentation" onClick={() => { setSelectedOffer(null); setQuote(null); }}>
            ...
            {quote && (
              <>
                <div className={styles.quoteRows}>
                  <div><span>Custo base</span><strong>{formatMoney(quote.quote.providerCostBrlCents)}</strong></div>
                  <div><span>Markup</span><strong>{formatMoney(quote.quote.markupCents)}</strong></div>
                  <div className={styles.quoteTotal}><span>Total</span><strong>{formatMoney(quote.quote.salePriceCents)}</strong></div>
                  <div><span>Seu saldo</span><strong>{formatMoney(quote.walletBalanceCents)}</strong></div>
                </div>
                <div className={quote.canAfford ? styles.okNotice : styles.warnNotice}>
                  {quote.canAfford ? "Seu saldo seria suficiente para este orçamento." : "Seu saldo seria insuficiente para este orçamento."}
                </div>
                <p className={styles.modalText}>Ao confirmar abaixo, faremos somente uma simulação da etapa de confirmação. Não haverá débito, reserva, compra ou chamada ao provider.</p>
...
function ActivationList({ activations, currency, compact = false }: { activations: Activation[]; currency: string; compact?: boolean }) {
  if (activations.length === 0) return <div className={styles.empty}>Você ainda não possui ativações.</div>;
  return (
