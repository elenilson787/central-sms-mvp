...
export function normalizeRentalEntries(payload: SmsPoolRentalList | unknown): Array<{ key: string; entry: SmsPoolRentalEntry }> {
  ...
  // SMSPool may wrap the useful array/object in an envelope such as { data: [...] }.
  // Unwrap known containers before interpreting object keys as rental IDs.
...
export function rentalPricingMap(payload: SmsPoolRentalPricing | unknown): Record<string, string | number> {
  ...
  if (isObject(payload)) {
    ...
    // Defensive fallback for an API response that is already a { "7": price, ... } map.
